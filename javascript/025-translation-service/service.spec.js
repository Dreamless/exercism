...
describe('Free service', () => {
  ...
  test('reject test', async () => {
    const actual = service.tryPromiseReject();
    const expected = 'error handler without reject';

    await expect(actual).rejects.toThrow(expected);
  });

  test('reject test2', async () => {
    const actual = service.tryPromiseReject2();
    const expected = 'error handler with reject';
